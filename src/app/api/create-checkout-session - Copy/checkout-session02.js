...
export const POST = withAuth(async (req, user) => {
  try {
    ...
    if (!activeLocation?.currentCountry) {
      return addCorsHeaders(
        NextResponse.json({ error: "No active country found for user." }, { status: 404 })
      );
    }

    // Fetch secret_key and currency from country table
    const country = await Country.findById(activeLocation.currentCountry)
      .select("secret_key currency -_id")
      .lean();

    if (!country?.secret_key) {
      return addCorsHeaders(
        NextResponse.json({ error: "Stripe secret key not found for country." }, { status: 500 })
      );
    }

    // Initialize Stripe with country-specific secret_key
    const stripe = new Stripe(country.secret_key);

    // Prepare Stripe line items
    const line_items = cartItems.map((item) => ({
      price_data: {
        currency: country.currency?.toLowerCase() || "", // Use country's currency
        product_data: {
          name: item.product?.title || item.title,
          images: item.product?.images || [], // Optional: pass product images
        },
        unit_amount: Math.round((item.product?.price || item.price) * 100), // amount in smallest unit
      },
